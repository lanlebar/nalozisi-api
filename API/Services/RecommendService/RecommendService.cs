...
namespace API.Services.RecommendService  
{
    public class RecommendService : IRecommnedService
    {
        ...
        public async Task<Recommendation> SetRecommendation(Recommendation recommendation)
        {
            ...
            var todaysRecommendation = await _context.Recommendation.FirstOrDefaultAsync(r => r.Date.Equals(recommendation.Date) && r.Type.Equals(recommendation.Type));
        ...
        {
            Recommendation recommendation = await _context.Recommendation.FirstOrDefaultAsync(r => r.Date.Equals(datetime) && r.Type.Equals(type)) ?? throw new ArgumentException();
            return recommendation;
        }

        public async Task DeleteRecommendation(DateOnly datetime, string type)
        {
            var recommendation = await _context.Recommendation.FirstOrDefaultAsync(r => r.Date.Equals(datetime) && r.Type.Equals(type));
